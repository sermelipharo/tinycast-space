...
| **Extension runtime fix.** `querystring.stringify` coerces `undefined` to `""` like Node does. | Without it, Google sign-in in extensions such as 2FA Code Finder fails with `invalid_client`. |
